  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8 px-4">
      <div className="max-w-4xl mx-auto">
        {/* Header */}
        <div className="mb-6">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Your First Workout</h1>
          <p className="text-gray-600">Let's get started on your transformation journey</p>
        </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {templates.map((template) => (
                  <button
                    ...
                    className="p-5 border-2 border-gray-300 rounded-lg hover:border-blue-600 hover:bg-blue-50 transition-all text-left"
                  >
            ...
            <div className="bg-white rounded-xl shadow-sm p-6">
              ...
              {session.exercises[currentExerciseIndex].sets.length > 0 && (
                <div className="space-y-2 mb-6">
                  <p className="text-sm font-medium text-gray-700 mb-2">Completed Sets</p>
                  {session.exercises[currentExerciseIndex].sets.map((set, idx) => (
                    <div
                      key={idx}
                      className="flex items-center justify-between p-3 bg-green-50 border border-green-200 rounded-lg"
                    >
                      <span className="text-sm text-gray-700">Set {set.setNumber}</span>
                      <span className="font-medium text-gray-900">
                        {set.weight} lbs × {set.reps} reps
                        {set.rpe && <span className="text-gray-600 ml-2">RPE: {set.rpe}</span>}
                      </span>
